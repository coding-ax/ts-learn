...
function del(x: number, y: number) {
    return x - y;
}
...
let myDel: (x: number, y: number) => number = function (x, y) {
    return x - y
}
// 使用问号让参数可选
let buildName: (fname: string, lname?: string) => string = function (firstName: string, lastName?: string): string {
...
function sayGoodBye(name: string = "AX", age: number):string {
...
// 写法 ...参数名:类型(数组)
// 必须加上... 否则就是当数组处理(解构赋值)
function watchArgs(name: string, ...args: string[]):void {
    console.log(args)    
}
